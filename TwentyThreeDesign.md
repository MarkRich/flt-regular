...
The verifier is instantiated twice for the two residue-degree-11 rational
primes in the full-field Minkowski sieve. `TwentyThreePrimeTwoCube.lean` uses
an irreducible degree-11 factor of `Φ₂₃` modulo `2`, while
`TwentyThreePrimeThreeCube.lean` uses one modulo `3`. Each module identifies
the corresponding Kummer--Dedekind prime ideal and proves its cube principal
from five exact integer-polynomial identities. Galois conjugacy transports
that relation to the other prime above the same rational prime, so the two
modules cover all prime ideals above `2` and `3`. They do not cover the
degree-one or degree-two branches of the sieve. The ramified branch is handled
separately in `TwentyThreeRamifiedPrime.lean`: Mathlib's cyclotomic
factorization identifies the unique prime above `23` with the principal ideal
generated by `zeta_23 - 1`, so its cube is principal without a certificate.
The ramified module also combines all three cases into the exact
one-prime-above callback shape, leaving only primes other than `2`, `3`, and
`23` as hypotheses.
...
1. The generic `CubicIdealCertificate` verifier, both residue-degree-11 cases,
   and the ramified case are complete. `TwentyThreePrimeTwoCube.lean` and
   `TwentyThreePrimeThreeCube.lean` prove that selected degree-11 factors
   modulo `2` and `3` define primes with principal cube. Deterministic SymPy
   witness finders and independent BigInt validators preserve the discovery
   and validation pipeline. Galois conjugacy covers the other prime above each
   rational prime. `TwentyThreeRamifiedPrime.lean` proves the unique prime
   above `23` is itself principal. These relations do not prove that they
...
4. If no compact unconditional presentation is available, formalize Kummer's
   ...
   mathematics.
5. Use the remaining 28,281-witness Minkowski route only as a last-resort
   generated proof, split into independently checked chunks from the start.
